...
const AuthPage = () => {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      // If popup flow returned tokens, AuthContext picks up the session.
      navigate("/", { replace: true });
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <main className="mx-auto max-w-md px-5 pt-6">
        <h1 className="mb-2 text-3xl">{mode === "signin" ? "Welcome back" : "Create your account"}</h1>
        <p className="mb-8 text-muted-foreground">
          {mode === "signin" ? "Sign in to save recipes and share your own." : "Join COOKED to share recipes and save favorites."}
        </p>
        ...
        <Card className="p-6 shadow-card">
          ...
          <Button variant="outline" onClick={handleGoogle} disabled={busy} className="w-full" size="lg">
